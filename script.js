import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
        ...
        function queueUiRefresh(force = false) {
            ...
            refreshTimer = setTimeout(() => {
                ...
                try {
                    ...
                    if (currentScreen === 'screen-admin-mitra' && typeof renderAdminPengajuanMitra === 'function') renderAdminPengajuanMitra();
                    if (currentScreen === 'screen-kemitraan-dashboard' && typeof initKemitraanDashboard === 'function') initKemitraanDashboard();
                    if (currentScreen === 'screen-kemitraan-daftar' && typeof renderKemitraanDaftarMitra === 'function') renderKemitraanDaftarMitra();
                    if (currentScreen === 'screen-kemitraan-riwayat' && typeof renderKemitraanRiwayatTrx === 'function') renderKemitraanRiwayatTrx();
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            onValue(ref(db, 'pengajuan_mitra'), (snapshot) => {
                cloudPengajuanMitra = snapshot.val() || {};
                queueUiRefresh();
            });
            ...
            if (savedSession) {
                ...
                if (userSession.role === 'owner') {
                    ...
                    if ((userSession.kategori || '').trim() === 'Petugas Kemitraan') {
                        launchApplicationSession("screen-kemitraan-dashboard");
                        setTimeout(() => {
                            if (typeof initKemitraanDashboard === 'function') initKemitraanDashboard();
                        }, 100);
                    } else {
                        launchApplicationSession("screen-admin-dashboard");
                        setTimeout(() => {
                    ...
                    if (snapshotManajemen && snapshotManajemen.exists()) {
                        ...
                        for (let key in dataManajemen) {
                            ...
                            if (manUser === userIn && manPass === passIn) {
                                ...
                                const kategoriLoginFixed = (m.kategori || "-").trim();
                                if (kategoriLoginFixed === 'Petugas Kemitraan') {
                                    // Petugas Kemitraan punya panel & tampilan tersendiri, terpisah dari admin/HRD/ops
                                    launchApplicationSession("screen-kemitraan-dashboard");
                                    setTimeout(() => {
                                        if (typeof initKemitraanDashboard === 'function') initKemitraanDashboard();
                                    }, 100);
        ...
        const SCREEN_META = {
            ...
            'screen-kemitraan-ajukan': { title: 'Ajukan Mitra Baru', icon: 'store' },
            'screen-kemitraan-daftar': { title: 'Daftar Mitra', icon: 'store' },
            'screen-kemitraan-riwayat': { title: 'Riwayat Transaksi Mitra', icon: 'history' }
        };
        ...
        window.navigateTo = function(screenId) {
            ...
            if (screenId === 'screen-admin-mitra') {
                setTimeout(() => {
                    if (typeof renderAdminPengajuanMitra === 'function') renderAdminPengajuanMitra();
                }, 100);
            }
            if (screenId === 'screen-kemitraan-dashboard') {
                setTimeout(() => {
                    if (typeof initKemitraanDashboard === 'function') initKemitraanDashboard();
                }, 100);
            }
            if (screenId === 'screen-kemitraan-daftar') {
                setTimeout(() => {
                    const bulanEl = document.getElementById('km-daftar-filter-bulan');
                    if (bulanEl && !bulanEl.value) bulanEl.value = getWibRawDate().slice(0, 7);
                    if (typeof renderKemitraanDaftarMitra === 'function') renderKemitraanDaftarMitra();
                }, 100);
            }
            if (screenId === 'screen-kemitraan-riwayat') {
                setTimeout(() => {
                    const today = getWibRawDate();
                    const tglEl = document.getElementById('km-riwayat-tgl');
                    if (tglEl && !tglEl.value) tglEl.value = today;
                    const bulanEl = document.getElementById('km-riwayat-bulan');
                    if (bulanEl && !bulanEl.value) bulanEl.value = today.slice(0, 7);
                    const cekTglEl = document.getElementById('km-cek-tgl');
                    if (cekTglEl && !cekTglEl.value) cekTglEl.value = today;
                    if (typeof populateKemitraanRiwayatKurirDropdown === 'function') populateKemitraanRiwayatKurirDropdown();
                    if (typeof renderKemitraanRiwayatTrx === 'function') renderKemitraanRiwayatTrx();
                }, 100);
            }
            ...
            if (appBar) {
                if (screenId === 'screen-login' || screenId === 'screen-dashboard' || screenId === 'screen-admin-dashboard' || screenId === 'screen-kemitraan-dashboard') {
            ...
            if (screenId === 'screen-dashboard' || screenId === 'screen-admin-dashboard' || screenId === 'screen-kemitraan-dashboard') {
                navigationHistory = [];
            } else {
        ...
        window.navigateBack = function() {
            if (navigationHistory.length > 0) {
                ...
                const appBar = document.getElementById('app-bar');
                if (appBar) {
                    if (prev === 'screen-dashboard' || prev === 'screen-admin-dashboard' || prev === 'screen-kemitraan-dashboard') {
                        appBar.classList.remove('flex');
                        appBar.classList.add('hidden');
                    } else {
                        appBar.classList.remove('hidden');
        ...
        window.backToDashboardRole = function() {
            if (userSession && userSession.role === 'owner') navigateTo('screen-admin-dashboard');
                ...
                if ((userSession.kategori || '').trim() === 'Petugas Kemitraan') navigateTo('screen-kemitraan-dashboard');
                else navigateTo('screen-admin-dashboard');
            }
        ...
        const KEMITRAAN_UPLOAD_ENDPOINT = "https://script.google.com/macros/s/AKfycbzL7wU9ktHQqekp6rpt34U8DUgB_6QcKZLVSNYONDwaR0-R2BRvK-WJjXjD-hb7vXEJ/exec";
        const KEMITRAAN_FOLDER = {
            ...
            fotoSurat: "1XZA3fpxVDTtsxBCUi66mLXl6gJDvIzTkFRFK7IYyedSWRWYemqw9ejQSD_aR8JgGnTvvdRHx"
        };
        // Simpan hasil kompres foto di memori (bukan localStorage), key: 'foto-lokasi' | 'foto-dokumentasi' | 'foto-surat'
        let kemitraanFotoBase64 = {};
        let kemitraanHpKaryawanList = [""];

        // Kompres gambar di sisi browser (resize max 1280px + kualitas 0.8) supaya ukuran file kecil tapi tetap jelas
        function kompresGambarKemitraan(file) {
            return new Promise((resolve, reject) => {
                // Jaring pengaman: kalau karena alasan apapun prosesnya tidak kunjung selesai,
                // jangan sampai menggantung tanpa batas - gagalkan dengan pesan yang jelas setelah 20 detik.
                const timeoutGuard = setTimeout(() => {
                    reject(new Error('Proses foto terlalu lama (timeout). Coba pakai foto dengan ukuran lebih kecil.'));
                }, 20000);
                const selesai = (fn) => (...args) => { clearTimeout(timeoutGuard); fn(...args); };

                const reader = new FileReader();
                reader.onload = (e) => {
                    const img = new Image();
                    img.onload = () => {
                        // Catatan: sebelumnya pakai requestAnimationFrame di sini, tapi rAF bisa tertunda lama
                        // atau tidak jalan sama sekali kalau tab/iframe preview sedang tidak aktif/tidak fokus.
                        // setTimeout jauh lebih bisa diandalkan untuk tetap jalan di kondisi seperti itu.
                        setTimeout(() => {
                            try {
                                const maxDim = 1000;
                                ...
                                // toBlob bersifat ASINKRON (tidak memblokir thread utama seperti toDataURL),
                                // jadi encode JPEG resolusi besar tidak akan bikin browser/preview "freeze".
                                canvas.toBlob((blob) => {
                                    if (!blob) { selesai(reject)(new Error('Gagal memproses gambar.')); return; }
                                    const previewUrl = URL.createObjectURL(blob);
                                    const blobReader = new FileReader();
                                    blobReader.onload = () => {
                                        selesai(resolve)({ base64: blobReader.result.split(',')[1], preview: previewUrl });
                                    };
                                    blobReader.onerror = () => selesai(reject)(new Error('Gagal membaca hasil kompres foto.'));
                    ...
                    img.onerror = () => selesai(reject)(new Error('Gagal memuat foto (file rusak atau format tidak didukung).'));
                    img.src = e.target.result;
                };
                reader.onerror = () => selesai(reject)(new Error('Gagal membaca file foto.'));
                reader.readAsDataURL(file);
            });
        }

        window.prosesFotoKemitraan = async function(inputEl, tipeFoto) {
            const file = inputEl.files && inputEl.files[0];
            if (!file) return;
            try {
                const hasil = await kompresGambarKemitraan(file);
                kemitraanFotoBase64[tipeFoto] = { base64: hasil.base64, fileName: file.name || `${tipeFoto}.jpg` };
                const preview = document.getElementById(`km-preview-${tipeFoto}`);
                const placeholder = document.getElementById(`km-placeholder-${tipeFoto}`);
                if (preview) { preview.src = hasil.preview; preview.classList.remove('hidden'); }
        ...
        async function uploadFotoKemitraanKeDrive(tipeFoto, folderId, namaFileFinal) {
            ...
            try {
                res = await fetch(KEMITRAAN_UPLOAD_ENDPOINT, {
                    method: "POST",
                ...
                if (networkErr.name === 'AbortError') {
                    throw new Error(`Upload ${tipeFoto} timeout (server tidak merespons dalam 30 detik). Coba lagi.`);
                }
                throw new Error(`Gagal terhubung ke server upload (${tipeFoto}): ${networkErr.message}`);
            } finally {
            ...
            try {
                ...
                // Respons bukan JSON (biasanya halaman error/izin dari Google) -> tampilkan mentahnya biar ketahuan penyebab aslinya
                throw new Error(`Upload ${tipeFoto} gagal (respons tidak valid): ${rawText.slice(0, 150)}`);
            }

            if (resData && resData.success) return resData.url || "";
            throw new Error(`Upload ${tipeFoto} gagal - ${(resData && resData.error) || 'penyebab tidak diketahui'} (folderId: ${folderId})`);
        }

        window.tambahHpKaryawanKemitraan = function() {
            kemitraanHpKaryawanList.push("");
            renderHpKaryawanKemitraan();
        };
        window.hapusHpKaryawanKemitraan = function(idx) {
            if (kemitraanHpKaryawanList.length <= 1) { kemitraanHpKaryawanList = [""]; }
            else kemitraanHpKaryawanList.splice(idx, 1);
            renderHpKaryawanKemitraan();
        };
        window.updateHpKaryawanKemitraan = function(idx, val) {
            kemitraanHpKaryawanList[idx] = val;
        };
        function renderHpKaryawanKemitraan() {
            const container = document.getElementById('km-hp-karyawan-list');
            if (!container) return;
            let html = '';
            kemitraanHpKaryawanList.forEach((val, idx) => {
                html += `
                <div class="flex items-center gap-1.5 km-hp-karyawan-row">
                    <input type="tel" value="${val ? val.replace(/"/g, '&quot;') : ''}" placeholder="08xxxxxxxxxx" oninput="updateHpKaryawanKemitraan(${idx}, this.value)" class="px-3 py-2 border rounded-xl text-xs dark:bg-darkBg dark:border-slate-700">
                    <button type="button" onclick="hapusHpKaryawanKemitraan(${idx})" class="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-950/30 text-rose-500"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>
                </div>`;
            });
        ...
        window.bukaFormAjukanMitraBaru = function() {
            resetFormAjukanMitraKemitraan();
            const modeEl = document.getElementById('km-ajukan-mode');
            const targetEl = document.getElementById('km-ajukan-target-key');
            if (modeEl) modeEl.value = 'baru';
            if (targetEl) targetEl.value = '';
            const titleEl = document.getElementById('km-ajukan-header-title');
            const subEl = document.getElementById('km-ajukan-header-sub');
            const btnTextEl = document.getElementById('km-btn-ajukan-text');
            if (titleEl) titleEl.innerText = 'Ajukan Mitra Baru';
            if (subEl) subEl.innerText = 'Lengkapi data calon mitra dengan benar';
            if (btnTextEl) btnTextEl.innerText = 'Ajukan Mitra Baru';
            navigateTo('screen-kemitraan-ajukan');
        };

        function resetFormAjukanMitraKemitraan() {
            ['km-owner-nama', 'km-nama-kedai', 'km-alamat-maps', 'km-hp-owner', 'km-tgl-bergabung', 'km-jam-buka', 'km-jam-tutup'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.value = '';
            });
            const setor = document.getElementById('km-tgl-setor');
            if (setor) setor.value = '';
            kemitraanHpKaryawanList = [""];
            renderHpKaryawanKemitraan();
            kemitraanFotoBase64 = {};
            ['foto-lokasi', 'foto-dokumentasi', 'foto-surat'].forEach(tipe => {
                const preview = document.getElementById(`km-preview-${tipe}`);
                const placeholder = document.getElementById(`km-placeholder-${tipe}`);
                if (preview) { preview.src = ''; preview.classList.add('hidden'); }
                if (placeholder) placeholder.classList.remove('hidden');
                const inputFile = document.getElementById(`km-input-${tipe}`);
                if (inputFile) inputFile.value = '';
            });
        }

        window.submitAjukanMitraBaru = async function() {
            const btn = document.getElementById('km-btn-ajukan-mitra');
            const btnText = document.getElementById('km-btn-ajukan-text');
            if (btn && btn.disabled) return;

            const mode = document.getElementById('km-ajukan-mode')?.value || 'baru';
            const targetMitraKey = document.getElementById('km-ajukan-target-key')?.value || '';

            const ownerNama = (document.getElementById('km-owner-nama').value || '').trim();
            const namaKedai = (document.getElementById('km-nama-kedai').value || '').trim();
            const alamatMaps = (document.getElementById('km-alamat-maps').value || '').trim();
            const hpOwner = (document.getElementById('km-hp-owner').value || '').trim();
            const tglBergabung = document.getElementById('km-tgl-bergabung').value || '';
            const tglSetor = document.getElementById('km-tgl-setor').value || '';
            const jamBuka = document.getElementById('km-jam-buka').value || '';
            const jamTutup = document.getElementById('km-jam-tutup').value || '';
            const hpKaryawan = kemitraanHpKaryawanList.map(v => (v || '').trim()).filter(v => v);

            if (!ownerNama || !namaKedai || !alamatMaps || !hpOwner || !tglBergabung || !tglSetor || !jamBuka || !jamTutup) {
                toast('Lengkapi semua data pemilik & operasional mitra terlebih dahulu!');
                return;
            }
            // Foto lokasi & dokumentasi wajib hanya untuk pengajuan mitra BARU.
            // Untuk ajukan perubahan, foto lama tetap dipakai kalau petugas tidak upload foto baru.
            if (mode === 'baru' && (!kemitraanFotoBase64['foto-lokasi'] || !kemitraanFotoBase64['foto-dokumentasi'])) {
                toast('Foto lokasi kedai dan foto dokumentasi wajib diisi!');
                return;
            }
            if (mode === 'perubahan' && !targetMitraKey) {
                toast('Data mitra yang diubah tidak ditemukan. Silakan buka ulang dari Daftar Mitra.');
                return;
            }
            ...
            console.log('[kemitraan] submit dimulai, mode:', mode);

            try {
                const wib = getWibDate();
                const tglRaw = getWibRawDate();
                const stamp = `${tglRaw}_${Date.now()}`;
                const dataLamaMitra = (mode === 'perubahan' && targetMitraKey && cloudMitraList[targetMitraKey]) ? cloudMitraList[targetMitraKey] : {};

                // PENTING: upload SATU PER SATU (bukan bersamaan) supaya tidak membentur limit
                // concurrent-execution Google Apps Script yang sering bikin gagal walau file sempat kebuat.
                let fotoLokasiUrl = dataLamaMitra.fotoLokasiUrl || "";
                if (kemitraanFotoBase64['foto-lokasi']) {
                    if (btnText) btnText.innerText = 'Mengunggah foto lokasi...';
                    console.log('[kemitraan] upload foto lokasi...');
                    fotoLokasiUrl = await uploadFotoKemitraanDenganRetry('foto-lokasi', KEMITRAAN_FOLDER.fotoLokasi, `lokasi_${stamp}.jpg`);
                    console.log('[kemitraan] foto lokasi selesai:', fotoLokasiUrl);
                }

                let fotoDokumentasiUrl = dataLamaMitra.fotoDokumentasiUrl || "";
                if (kemitraanFotoBase64['foto-dokumentasi']) {
                    if (btnText) btnText.innerText = 'Mengunggah foto dokumentasi...';
                    console.log('[kemitraan] upload foto dokumentasi...');
                    fotoDokumentasiUrl = await uploadFotoKemitraanDenganRetry('foto-dokumentasi', KEMITRAAN_FOLDER.fotoDokumentasi, `dokumentasi_${stamp}.jpg`);
                    console.log('[kemitraan] foto dokumentasi selesai:', fotoDokumentasiUrl);
                }

                let fotoSuratUrl = dataLamaMitra.fotoSuratUrl || "";
                if (kemitraanFotoBase64['foto-surat']) {
                    if (btnText) btnText.innerText = 'Mengunggah surat pernyataan...';
                    console.log('[kemitraan] upload foto surat...');
                    fotoSuratUrl = await uploadFotoKemitraanDenganRetry('foto-surat', KEMITRAAN_FOLDER.fotoSurat, `suratpernyataan_${stamp}.jpg`);
                    console.log('[kemitraan] foto surat selesai:', fotoSuratUrl);
                }

                const payload = {
                    ownerNama,
                    namaKedai,
                    nama: namaKedai, // kompatibel dengan skema mitra lama (m.nama)
                    alamatMaps,
                    hpOwner,
                    hpKaryawan,
                    tglBergabung,
                    ...
                    fotoSuratUrl,
                    status: 'pending',
                    tipePengajuan: mode === 'perubahan' ? 'perubahan' : 'baru',
                    targetMitraKey: mode === 'perubahan' ? targetMitraKey : '',
                    diajukanOlehUsername: userSession.username,
                    diajukanOlehNama: userSession.nama || '-',
                    waktuPengajuan: tglRaw + ' ' + wib.toTimeString().split(' ')[0]
                };

                if (btnText) btnText.innerText = 'Menyimpan data pengajuan...';
                console.log('[kemitraan] mulai simpan pengajuan ke Firebase...', payload);

                // Firebase push() sendiri tidak punya batas waktu bawaan - kalau koneksi
                // websocket-nya tersendat di sandbox/preview, bisa menggantung tanpa pernah selesai.
                // Race dengan timeout supaya paling lama nunggu 20 detik, lalu kasih pesan jelas.
                await Promise.race([
                    push(ref(db, 'pengajuan_mitra'), payload),
                    new Promise((_, rej) => setTimeout(() => rej(new Error('Timeout menyimpan ke database (20 detik). Cek koneksi internet lalu coba lagi.')), 20000))
                ]);

                console.log('[kemitraan] pengajuan berhasil tersimpan ke Firebase.');

                // PENTING: begitu push() ke Firebase ini sukses, pengajuan SUDAH TERSIMPAN.
                // Langkah selanjutnya (reset form, pindah halaman, render dashboard) sengaja
                // dipisah di luar try/catch ini, supaya kalau proses render sempat lambat,
                // itu TIDAK membuat notifikasi salah bilang "gagal mengirim" padahal datanya sudah masuk.
                toast(mode === 'perubahan' ? 'Pengajuan perubahan data mitra berhasil dikirim! Menunggu persetujuan Admin.' : 'Pengajuan mitra baru berhasil dikirim! Menunggu persetujuan Admin.');

                setTimeout(() => {
                    try {
                        resetFormAjukanMitraKemitraan();
                        navigateTo('screen-kemitraan-dashboard');
                    } catch (uiErr) {
                        // Data sudah tersimpan (push sudah sukses di atas), ini cuma gagal pindah tampilan -
                        // jadi cukup arahkan manual, jangan bikin user kira pengajuannya gagal terkirim.
                        console.warn('Gagal update tampilan setelah submit (data tetap tersimpan):', uiErr);
                        toast('Pengajuan sudah tersimpan. Kembali ke dashboard secara manual bila layar tidak berpindah.');
                    }
                }, 0);
            } catch (err) {
                console.error('[kemitraan] submit gagal:', err);
                const pesan = (err && err.message) || 'Terjadi kesalahan.';
                if (/preview stopped|infinite loop/i.test(pesan)) {
                    // Error ini dari watchdog preview (deteksi proses lambat), BUKAN dari upload/simpan data itu sendiri.
                    // Catatan: pengajuan yang BELUM disetujui Admin tidak akan muncul di "Daftar Mitra" (itu daftar mitra yang sudah aktif) -
                    // cek di kartu "Pengajuan Pending" pada Dashboard untuk memastikan datanya benar-benar sudah/belum tersimpan.
                    toast('Preview sempat lambat merespons. Cek kartu "Pengajuan Pending" di Dashboard untuk memastikan sudah masuk sebelum kirim ulang.');
                } else {
                    toast('Gagal mengirim pengajuan: ' + pesan);
                }
            } finally {
                if (btn) btn.disabled = false;
                if (btnText) btnText.innerText = mode === 'perubahan' ? 'Ajukan Perubahan' : 'Ajukan Mitra Baru';
            }
        };

        window.initKemitraanDashboard = function() {
            renderHpKaryawanKemitraan();
            const totalMitraEl = document.getElementById('km-dash-total-mitra');
            if (totalMitraEl) totalMitraEl.innerText = Object.keys(cloudMitraList || {}).length;

            let totalTrxHariIni = 0;
            const tglSkrg = getWibRawDate();
            for (let k in cloudLogMitra) {
                const log = cloudLogMitra[k];
                if (log && log.tglRaw === tglSkrg) totalTrxHariIni += (parseInt(log.trxInput) || 0);
            }
            const trxEl = document.getElementById('km-dash-trx-hari-ini');
            if (trxEl) trxEl.innerText = totalTrxHariIni;

            get(ref(db, 'pengajuan_mitra')).then((snap) => {
                const data = snap.val() || {};
                const entries = Object.entries(data);
                const pendingCount = entries.filter(([_, v]) => v && v.status === 'pending').length;
                const pendingEl = document.getElementById('km-dash-total-pending');
                if (pendingEl) pendingEl.innerText = pendingCount;

                const milikSaya = entries
                    .filter(([_, v]) => v && v.diajukanOlehUsername === (userSession && userSession.username))
                    .sort((a, b) => (b[1].waktuPengajuan || '').localeCompare(a[1].waktuPengajuan || ''))
                    .slice(0, 5);

                const container = document.getElementById('km-dash-pengajuan-terbaru');
                if (!container) return;
                if (milikSaya.length === 0) {
                    container.innerHTML = `<p class="text-[11px] text-slate-400 text-center py-3">Belum ada pengajuan.</p>`;
                    return;
                }
                container.innerHTML = milikSaya.map(([key, v]) => {
                    const statusBadge = v.status === 'approved'
                        ? `<span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">Disetujui</span>`
                        : v.status === 'rejected'
                            ? `<span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400">Ditolak</span>`
                            : `<span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">Pending</span>`;
                    return `
                    <div class="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        <div class="min-w-0">
                            <p class="text-[11px] font-bold text-slate-700 dark:text-white truncate">${v.namaKedai || '-'}</p>
                            <p class="text-[9px] text-slate-400">${v.tipePengajuan === 'perubahan' ? 'Ajukan Perubahan' : 'Mitra Baru'}</p>
                        </div>
                        ${statusBadge}
                    </div>`;
                }).join('');
        ...
        // Klik kartu "Total Mitra" -> lihat semua mitra (filter bulan dikosongkan biar totalnya match)
        // Klik kartu "Total Mitra" -> tampilkan popup daftar semua mitra
        window.bukaDetailTotalMitra = function() {
            const existing = document.getElementById('popup-detail-total-mitra');
            if (existing) existing.remove();

            const list = Object.entries(cloudMitraList || {}).filter(([_, m]) => m && m.nama);
            list.sort((a, b) => (a[1].namaKedai || a[1].nama || '').localeCompare(b[1].namaKedai || b[1].nama || ''));

            const popup = document.createElement('div');
            popup.id = 'popup-detail-total-mitra';
            popup.className = 'fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4';
            popup.innerHTML = `
                <div class="bg-white dark:bg-darkCard w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-4 space-y-3 max-h-[80vh] overflow-y-auto">
                    <div class="flex items-center justify-between">
                        <h4 class="font-bold text-sm text-slate-800 dark:text-white">Total Mitra (${list.length})</h4>
                        <button onclick="document.getElementById('popup-detail-total-mitra').remove()" class="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500"><i data-lucide="x" class="w-3.5 h-3.5"></i></button>
                    </div>
                    <div class="space-y-1.5">
                        ${list.length === 0 ? '<p class="text-[11px] text-slate-400 text-center py-4">Belum ada data mitra.</p>' : list.map(([_, m], idx) => `
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                <p class="text-[11px] font-bold text-slate-700 dark:text-white truncate">${idx + 1}. ${m.namaKedai || m.nama}</p>
                                <p class="text-[9px] text-slate-400 mt-0.5">Owner: ${m.ownerNama || '-'} • Bergabung: ${m.tglBergabung || '-'}</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        // Klik kartu "Pengajuan Pending" -> tampilkan popup daftar semua pengajuan pending (dari semua petugas)
        window.bukaDetailPengajuanPending = function() {
            const existing = document.getElementById('popup-detail-pending');
            if (existing) existing.remove();

            get(ref(db, 'pengajuan_mitra')).then((snap) => {
                const data = snap.val() || {};
                const entries = Object.entries(data).filter(([_, v]) => v && v.status === 'pending');
                entries.sort((a, b) => (b[1].waktuPengajuan || '').localeCompare(a[1].waktuPengajuan || ''));

                const popup = document.createElement('div');
                popup.id = 'popup-detail-pending';
                popup.className = 'fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4';
                popup.innerHTML = `
                    <div class="bg-white dark:bg-darkCard w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-4 space-y-3 max-h-[80vh] overflow-y-auto">
                        <div class="flex items-center justify-between">
                            <h4 class="font-bold text-sm text-slate-800 dark:text-white">Pengajuan Pending (${entries.length})</h4>
                            <button onclick="document.getElementById('popup-detail-pending').remove()" class="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500"><i data-lucide="x" class="w-3.5 h-3.5"></i></button>
                        </div>
                        <div class="space-y-1.5">
                            ${entries.length === 0 ? '<p class="text-[11px] text-slate-400 text-center py-4">Tidak ada pengajuan pending.</p>' : entries.map(([_, v]) => `
                                <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                    <div class="flex items-start justify-between gap-2">
                                        <p class="text-[11px] font-bold text-slate-700 dark:text-white truncate">${v.namaKedai || '-'}</p>
                                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full shrink-0 ${v.tipePengajuan === 'perubahan' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400' : 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400'}">${v.tipePengajuan === 'perubahan' ? 'Perubahan' : 'Mitra Baru'}</span>
                                    </div>
                                    <p class="text-[9px] text-slate-400 mt-0.5">Oleh: ${v.diajukanOlehNama || '-'} • ${v.waktuPengajuan || '-'}</p>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ...
                document.body.appendChild(popup);
                if (window.lucide) lucide.createIcons();
            }).catch(() => toast('Gagal memuat data pengajuan.'));
        };

        // Klik kartu "Trx Hari Ini" -> tampilkan popup daftar transaksi hari ini
        window.bukaDetailTrxHariIni = function() {
            const existing = document.getElementById('popup-detail-trx-hari-ini');
            if (existing) existing.remove();
            ...
            for (let k in cloudLogMitra) {
                const log = cloudLogMitra[k];
                if (log && log.tglRaw === tglSkrg) rows.push(log);
            }
            rows.sort((a, b) => (b.waktu || '').localeCompare(a.waktu || ''));
            const totalTrx = rows.reduce((sum, r) => sum + (parseInt(r.trxInput) || 0), 0);

            const popup = document.createElement('div');
            popup.id = 'popup-detail-trx-hari-ini';
            popup.className = 'fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4';
            popup.innerHTML = `
                <div class="bg-white dark:bg-darkCard w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-4 space-y-3 max-h-[80vh] overflow-y-auto">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="font-bold text-sm text-slate-800 dark:text-white">Trx Hari Ini (${totalTrx} Trx)</h4>
                            <p class="text-[10px] text-slate-400">${tglSkrg} • ${rows.length} transaksi tercatat</p>
                        </div>
                        <button onclick="document.getElementById('popup-detail-trx-hari-ini').remove()" class="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500"><i data-lucide="x" class="w-3.5 h-3.5"></i></button>
                    </div>
                    <div class="space-y-1.5">
                        ${rows.length === 0 ? '<p class="text-[11px] text-slate-400 text-center py-4">Belum ada transaksi hari ini.</p>' : rows.map(log => `
                            <div class="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-2 rounded-lg text-[11px] border border-slate-100 dark:border-slate-700/50">
                                <div class="min-w-0">
                                    <span class="font-bold text-slate-700 dark:text-white">${log.mitraNama || '-'}</span>
                                    <p class="text-[9px] text-slate-400">Kurir: ${log.kurirNama || '-'} • ${log.waktu || ''}</p>
                                </div>
                                <span class="text-teal-600 dark:text-teal-400 font-bold shrink-0">${log.trxInput || 0} Trx</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        // ================= ADMIN: APPROVAL PENGAJUAN MITRA (KEMITRAAN) ================= //
        window.renderAdminPengajuanMitra = function() {
            const container = document.getElementById('container-admin-pengajuan-mitra');
            const badge = document.getElementById('am-badge-pengajuan-pending');
            if (!container) return;

            const entries = Object.entries(cloudPengajuanMitra || {}).filter(([_, v]) => v && v.status === 'pending');
            entries.sort((a, b) => (b[1].waktuPengajuan || '').localeCompare(a[1].waktuPengajuan || ''));

            if (badge) {
                if (entries.length > 0) { badge.innerText = entries.length; badge.classList.remove('hidden'); }
                else badge.classList.add('hidden');
            }

            if (entries.length === 0) {
                container.innerHTML = `<p class="text-[11px] text-slate-400 text-center py-3">Tidak ada pengajuan pending.</p>`;
                return;
            }

            container.innerHTML = entries.map(([key, v]) => {
                const tipeLabel = v.tipePengajuan === 'perubahan' ? 'Ajukan Perubahan' : 'Mitra Baru';
                const tipeBadgeTone = v.tipePengajuan === 'perubahan' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400' : 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400';
                const fotoRow = (v.fotoLokasiUrl || v.fotoDokumentasiUrl)
                    ? `<div class="flex gap-2 pt-1">
                        ${v.fotoLokasiUrl ? `<a href="${v.fotoLokasiUrl}" target="_blank" class="flex-1 text-center py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[9px] font-bold text-slate-600 dark:text-slate-300">Foto Lokasi</a>` : ''}
                        ${v.fotoDokumentasiUrl ? `<a href="${v.fotoDokumentasiUrl}" target="_blank" class="flex-1 text-center py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[9px] font-bold text-slate-600 dark:text-slate-300">Foto Dokumentasi</a>` : ''}
                        ${v.fotoSuratUrl ? `<a href="${v.fotoSuratUrl}" target="_blank" class="flex-1 text-center py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[9px] font-bold text-slate-600 dark:text-slate-300">Surat Pernyataan</a>` : ''}
                       </div>`
                    : '';
                return `
                <div class="p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 text-xs space-y-1.5">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <h5 class="font-bold text-slate-800 dark:text-white truncate">${v.namaKedai || '-'}</h5>
                            <p class="text-[9px] text-slate-400">Owner: ${v.ownerNama || '-'} • HP: ${v.hpOwner || '-'}</p>
                            <p class="text-[9px] text-slate-400">Diajukan oleh: ${v.diajukanOlehNama || '-'} • ${v.waktuPengajuan || '-'}</p>
                        </div>
                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full shrink-0 ${tipeBadgeTone}">${tipeLabel}</span>
                    </div>
                    ${fotoRow}
                    <div class="grid grid-cols-2 gap-2 pt-1">
                        <button onclick="approvePengajuanMitra('${key}')" class="py-1.5 rounded-lg bg-emerald-600 text-white font-bold text-[10px] uppercase">Setujui</button>
                        <button onclick="rejectPengajuanMitra('${key}')" class="py-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 font-bold text-[10px] uppercase">Tolak</button>
                    </div>
                </div>`;
            }).join('');
        };

        window.approvePengajuanMitra = async function(key) {
            const v = cloudPengajuanMitra && cloudPengajuanMitra[key];
            if (!v) return toast('Data pengajuan tidak ditemukan.');
            const ok = await showConfirm(`Setujui pengajuan "${v.namaKedai || '-'}" (${v.tipePengajuan === 'perubahan' ? 'Ajukan Perubahan' : 'Mitra Baru'})?`);
            if (!ok) return;
            ...
            try {
                const dataMitra = {
                    ...
                    alamat: v.alamatMaps || '',
                    hp: v.hpOwner || '',
                    ownerNama: v.ownerNama || '',
                    namaKedai: v.namaKedai || '',
                    alamatMaps: v.alamatMaps || '',
                    hpOwner: v.hpOwner || '',
                    hpKaryawan: v.hpKaryawan || [],
                    tglBergabung: v.tglBergabung || '',
                    tglSetor: v.tglSetor || '',
                    jamBuka: v.jamBuka || '',
                    jamTutup: v.jamTutup || '',
                    fotoLokasiUrl: v.fotoLokasiUrl || '',
                    fotoDokumentasiUrl: v.fotoDokumentasiUrl || '',
                    fotoSuratUrl: v.fotoSuratUrl || ''
                };

                if (v.tipePengajuan === 'perubahan' && v.targetMitraKey) {
                    // Ajukan perubahan: update record mitra yang sudah ada, target trx bulanan lama tetap dipertahankan
                    const existing = (cloudMitraList && cloudMitraList[v.targetMitraKey]) || {};
                    await update(ref(db, `mitra/${v.targetMitraKey}`), { ...dataMitra, target: existing.target || 0 });
                } else {
                    dataMitra.target = 0;
                    await push(ref(db, 'mitra'), dataMitra);
                }

                await update(ref(db, `pengajuan_mitra/${key}`), { status: 'approved' });
                toast('Pengajuan mitra berhasil disetujui!');
            } catch (err) {
                toast('Gagal menyetujui pengajuan: ' + (err && err.message ? err.message : 'Terjadi kesalahan.'));
            }
        };

        window.rejectPengajuanMitra = async function(key) {
            const ok = await showConfirm('Tolak pengajuan ini? Petugas kemitraan akan melihat status ditolak.');
            if (!ok) return;
            update(ref(db, `pengajuan_mitra/${key}`), { status: 'rejected' }).then(() => {
                toast('Pengajuan ditolak.');
            }).catch(err => toast('Gagal menolak pengajuan: ' + err.message));
        };

        // ================= PETUGAS KEMITRAAN: DAFTAR MITRA + AJUKAN PERUBAHAN ================= //
        window.toggleKemitraanMitraOpen = function() {
            const container = document.getElementById('km-container-daftar-mitra');
            const btn = document.getElementById('btn-toggle-km-mitra-text');
            ...
            if (!isOpen) {
                renderKemitraanDaftarMitra();
            } else {
                container.innerHTML = '';
            }
        };

        window.renderKemitraanDaftarMitra = function() {
            const container = document.getElementById('km-container-daftar-mitra');
            if (!container) return;
            if (container.dataset.open !== '1') return;

            const searchKey = normalizeNama(document.getElementById('km-daftar-search')?.value || '');
            const filterBulan = document.getElementById('km-daftar-filter-bulan')?.value || '';

            // Hitung total trx per mitra SEKALI SAJA (bukan diulang per-mitra) supaya tidak berat kalau datanya besar.
            const trxPerMitra = {}; // namaNorm -> { total, bulan }
            for (let logKey in cloudLogMitra) {
                const log = cloudLogMitra[logKey];
                if (!log || !log.mitraNama) continue;
                const namaNorm = normalizeNama(log.mitraNama);
                if (!trxPerMitra[namaNorm]) trxPerMitra[namaNorm] = { total: 0, bulan: 0 };
                const trx = parseInt(log.trxInput) || 0;
                trxPerMitra[namaNorm].total += trx;
                if (log.tglRaw && log.tglRaw.substring(0, 7) === filterBulan) trxPerMitra[namaNorm].bulan += trx;
            }
            ...
            for (let key in cloudMitraList) {
                ...
                const rekapTrx = trxPerMitra[namaMitra] || { total: 0, bulan: 0 };
                const totalTrxMenyeluruh = rekapTrx.total;
                const totalTrxFilterBulan = rekapTrx.bulan;

                const mapsLink = getMapsLink(m.alamatMaps || m.alamat);
                const jamOperasional = (m.jamBuka || m.jamTutup) ? `${m.jamBuka || '-'} - ${m.jamTutup || '-'}` : '-';
                const jumlahHpKaryawan = Array.isArray(m.hpKaryawan) ? m.hpKaryawan.length : 0;

                list.push(`
                    <div class="bg-white dark:bg-darkCard p-3 rounded-xl border border-slate-100 dark:border-slate-800 text-xs space-y-2 shadow-sm">
                        <div class="flex justify-between items-start gap-2">
                            <div class="min-w-0">
                                <h5 class="font-bold text-slate-800 dark:text-white truncate">${nomorUrut}. ${m.namaKedai || m.nama}</h5>
                                <p class="text-[10px] text-slate-400">Owner: ${m.ownerNama || '-'} • ${m.hpOwner || m.hp || '-'}</p>
                                <a href="${mapsLink}" target="_blank" class="text-[10px] text-blue-600 dark:text-blue-400 hover:underline block mt-0.5 truncate">Lihat Alamat / Maps</a>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg text-[10.5px] border border-slate-100 dark:border-slate-800">
                            <div><span class="text-slate-400 block text-[9px] uppercase">Tgl Bergabung</span><span class="font-bold text-slate-700 dark:text-slate-200">${m.tglBergabung || '-'}</span></div>
                            <div><span class="text-slate-400 block text-[9px] uppercase">Setor Tgl</span><span class="font-bold text-slate-700 dark:text-slate-200">${m.tglSetor || '-'}</span></div>
                            <div><span class="text-slate-400 block text-[9px] uppercase">Jam Operasional</span><span class="font-bold text-slate-700 dark:text-slate-200">${jamOperasional}</span></div>
                            <div><span class="text-slate-400 block text-[9px] uppercase">HP Karyawan</span><span class="font-bold text-slate-700 dark:text-slate-200">${jumlahHpKaryawan} nomor</span></div>
                            <div class="col-span-2"><span class="text-slate-400 block text-[9px] uppercase">${filterBulan ? 'Trx Bulan Ini' : 'Total Trx'}</span><span class="font-extrabold text-teal-600 dark:text-teal-400">${filterBulan ? totalTrxFilterBulan : totalTrxMenyeluruh} Trx</span></div>
                        </div>
                        <button onclick="bukaAjukanPerubahanMitra('${key}')" class="w-full py-2 rounded-lg bg-slate-800 dark:bg-slate-700 text-white text-[10px] font-bold uppercase flex items-center justify-center gap-1.5">
                            <i data-lucide="pencil" class="w-3 h-3"></i> Ajukan Perubahan Data
                        </button>
                    </div>
                `);
                nomorUrut++;
            ...
            container.innerHTML = list.join('') || '<div class="text-center text-xs text-slate-400 py-4">Belum ada data mitra.</div>';
            if (window.lucide) lucide.createIcons();
        };

        window.bukaAjukanPerubahanMitra = function(key) {
            const m = cloudMitraList && cloudMitraList[key];
            if (!m) return toast('Data mitra tidak ditemukan.');

            resetFormAjukanMitraKemitraan();
            document.getElementById('km-ajukan-mode').value = 'perubahan';
            document.getElementById('km-ajukan-target-key').value = key;
            document.getElementById('km-ajukan-header-title').innerText = 'Ajukan Perubahan Data Mitra';
            document.getElementById('km-ajukan-header-sub').innerText = 'Perubahan menunggu persetujuan Admin';
            document.getElementById('km-btn-ajukan-text').innerText = 'Ajukan Perubahan';

            document.getElementById('km-owner-nama').value = m.ownerNama || '';
            document.getElementById('km-nama-kedai').value = m.namaKedai || m.nama || '';
            document.getElementById('km-alamat-maps').value = m.alamatMaps || m.alamat || '';
            document.getElementById('km-hp-owner').value = m.hpOwner || m.hp || '';
            document.getElementById('km-tgl-bergabung').value = m.tglBergabung || '';
            document.getElementById('km-tgl-setor').value = m.tglSetor || '';
            document.getElementById('km-jam-buka').value = m.jamBuka || '';
            document.getElementById('km-jam-tutup').value = m.jamTutup || '';

            kemitraanHpKaryawanList = (Array.isArray(m.hpKaryawan) && m.hpKaryawan.length > 0) ? [...m.hpKaryawan] : [""];
            renderHpKaryawanKemitraan();

            // Foto lama tetap ditampilkan sebagai preview; kalau petugas tidak upload baru, foto lama tetap dipakai
            [
                ['foto-lokasi', m.fotoLokasiUrl],
                ['foto-dokumentasi', m.fotoDokumentasiUrl],
                ['foto-surat', m.fotoSuratUrl]
            ].forEach(([tipe, url]) => {
                if (!url) return;
                const preview = document.getElementById(`km-preview-${tipe}`);
                const placeholder = document.getElementById(`km-placeholder-${tipe}`);
        ...
        // ================= PETUGAS KEMITRAAN: RIWAYAT TRANSAKSI + CEK BELUM INPUT ================= //
        window.populateKemitraanRiwayatKurirDropdown = function() {
            const dropdown = document.getElementById('km-riwayat-filter-kurir');
            if (!dropdown) return;
        ...
        window.resetFilterRiwayatKemitraan = function() {
            const today = getWibRawDate();
            document.getElementById('km-riwayat-tgl').value = today;
            document.getElementById('km-riwayat-bulan').value = today.slice(0, 7);
            document.getElementById('km-riwayat-search').value = '';
            const dd = document.getElementById('km-riwayat-filter-kurir');
            if (dd) dd.value = 'semua';
            renderKemitraanRiwayatTrx();
        };

        window.toggleKemitraanRiwayatTrxOpen = function() {
            const box = document.getElementById('box-km-riwayat-trx');
            const btn = document.getElementById('btn-toggle-km-riwayat-text');
            if (!box || !btn) return;
            ...
            btn.innerText = isOpen ? 'Tutup' : 'Buka';

            if (isOpen) {
                populateKemitraanRiwayatKurirDropdown();
                renderKemitraanRiwayatTrx();
            }
        };

        window.renderKemitraanRiwayatTrx = function() {
            const container = document.getElementById('km-container-riwayat-trx');
            const box = document.getElementById('box-km-riwayat-trx');
            if (!container) return;
            if (box && box.classList.contains('hidden')) return;

            const tgl = document.getElementById('km-riwayat-tgl')?.value || '';
            const bulan = document.getElementById('km-riwayat-bulan')?.value || '';
            const search = normalizeNama(document.getElementById('km-riwayat-search')?.value || '');
            const filterKurir = document.getElementById('km-riwayat-filter-kurir')?.value || 'semua';

            const rows = [];
            for (let k in cloudLogMitra) {
                const log = cloudLogMitra[k];
                if (!log) continue;
                if (filterKurir !== 'semua' && log.kurirNama !== filterKurir) continue;
                if (tgl && log.tglRaw !== tgl) continue;
                if (bulan && (!log.tglRaw || log.tglRaw.substring(0, 7) !== bulan)) continue;
                if (search) {
                    const namaMitra = normalizeNama(log.mitraNama || '');
                    const namaKurir = normalizeNama(log.kurirNama || '');
                    if (!namaMitra.includes(search) && !namaKurir.includes(search)) continue;
                }
                rows.push(log);
            }

            rows.sort((a, b) => `${b.tglRaw || ''} ${b.waktu || ''}`.localeCompare(`${a.tglRaw || ''} ${a.waktu || ''}`));

            if (rows.length === 0) {
                container.innerHTML = '<div class="text-center text-xs text-slate-400 py-4">Tidak ada data transaksi ditemukan.</div>';
                return;
            }
            ...
            container.innerHTML = rows.map(log => `
                <div class="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-2 rounded-lg text-[11px] border border-slate-100 dark:border-slate-700/50">
                    <div class="min-w-0">
                        ...
                        <span class="text-teal-600 dark:text-teal-400 font-bold"> (${log.trxInput || 0} Trx)</span>
                        <p class="text-[9px] text-slate-400">Kurir: ${log.kurirNama || '-'} • ${log.tglRaw || '-'} ${log.waktu || ''}</p>
                    </div>
                </div>
            `).join('');
        };

        // Helper: cek status off/izin/sakit seorang kurir (by nama) pada tanggal tertentu.
        // Mengembalikan label jenisnya, atau null kalau kurir tidak off/izin/sakit (berarti seharusnya bertugas).
        function cekStatusOffKurirPadaTanggal(namaKurirNorm, tglCek) {
            for (let k in cloudJadwalOff) {
                const j = cloudJadwalOff[k];
                if (!j) continue;
                const jenisOff = (j.jenisOff || '').toString().trim().toLowerCase();
                if (!['off reguler', 'izin', 'sakit'].includes(jenisOff)) continue;

                const namaKurir = normalizeNama(j.nama || j.namaKurir || j.kurirNama || '');
                if (namaKurir !== namaKurirNorm) continue;

                const tglMulai = (j.tanggalMulai || '').trim();
                const tglSelesai = (j.tanggalSelesai || tglMulai || '').trim();
                if (!tglMulai) continue;
                if (tglCek < tglMulai || tglCek > tglSelesai) continue;

                if (jenisOff === 'off reguler') return 'Off';
                if (jenisOff === 'izin') return 'Izin';
                if (jenisOff === 'sakit') return 'Sakit';
            }
            for (let k in cloudAbsensiList) {
                const a = cloudAbsensiList[k];
                if (!a) continue;
                const tglAbsen = (a.tglRaw || a.tanggal || '').toString().trim();
                if (tglAbsen !== tglCek) continue;
                if (normalizeNama(a.nama || a.namaKurir || '') !== namaKurirNorm) continue;
        ...
        // Cek Kurir Trx: menampilkan 3 kelompok pada tanggal tertentu -
        // (1) sudah input trx, (2) belum input trx (padahal seharusnya bertugas),
        // (3) sedang off/izin/sakit (jadi TIDAK dianggap belum input trx).
        window.cekKurirTrx = function() {
            const tglCek = document.getElementById('km-cek-tgl')?.value || getWibRawDate();
            const container = document.getElementById('km-container-cek-trx');
            if (!container) return;
            document.getElementById('km-cek-tgl').value = tglCek;

            // Kurir yang sudah input trx mitra pada tanggal tsb (+ hitung total trx-nya)
            const sudahInputMap = {}; // namaNorm -> { nama, totalTrx }
            for (let k in cloudLogMitra) {
                const log = cloudLogMitra[k];
                if (!log || log.tglRaw !== tglCek || !log.kurirNama) continue;
                const namaNorm = normalizeNama(log.kurirNama);
                if (!sudahInputMap[namaNorm]) sudahInputMap[namaNorm] = { nama: log.kurirNama, totalTrx: 0 };
                sudahInputMap[namaNorm].totalTrx += (parseInt(log.trxInput) || 0);
            }

            const sudahInput = [];
            const belumInput = [];
            const tidakHadir = []; // { nama, jenis }

            for (let k in cloudKurirList) {
                const kur = cloudKurirList[k];
                if (!kur || kur.role === 'admin' || !kur.nama) continue;
                if ((kur.status || 'aktif') !== 'aktif') continue;

                const namaNorm = normalizeNama(kur.nama);

                if (sudahInputMap[namaNorm]) {
                    sudahInput.push(sudahInputMap[namaNorm]);
                    continue;
                }

                const jenisOff = cekStatusOffKurirPadaTanggal(namaNorm, tglCek);
                if (jenisOff) {
                    tidakHadir.push({ nama: kur.nama, jenis: jenisOff });
                } else {
                    belumInput.push(kur.nama);
                }
            }

            const escNama = (s) => (s || '').replace(/'/g, "\\'");

            const sectionSudah = `
                <div>
                    <h5 class="text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 mb-1.5 flex items-center gap-1"><i data-lucide="user-round-check" class="w-3.5 h-3.5"></i> Sudah Input Trx (${sudahInput.length})</h5>
                    <div class="space-y-1.5">
                        ${sudahInput.length === 0 ? '<p class="text-[10px] text-slate-400 pl-1">Belum ada.</p>' : sudahInput.map(x => `
                            <button onclick="tampilkanDetailTrxKurir('${escNama(x.nama)}', '${tglCek}')" class="w-full flex items-center justify-between gap-2 bg-emerald-50 dark:bg-emerald-950/30 p-2 rounded-lg border border-emerald-100 dark:border-emerald-900/40 text-left active:scale-[0.98] transition-transform">
                                <span class="text-[11px] font-bold text-emerald-700 dark:text-emerald-400">${x.nama}</span>
                                <span class="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 shrink-0">${x.totalTrx} Trx &rsaquo;</span>
                            </button>
                        `).join('')}
                    </div>
                </div>`;
            ...
            const sectionBelum = `
                <div>
                    <h5 class="text-[10px] font-bold uppercase tracking-wide text-rose-600 dark:text-rose-400 mb-1.5 flex items-center gap-1"><i data-lucide="user-round-x" class="w-3.5 h-3.5"></i> Belum Input Trx (${belumInput.length})</h5>
                    <div class="space-y-1.5">
                        ${belumInput.length === 0 ? '<p class="text-[10px] text-slate-400 pl-1">Tidak ada.</p>' : belumInput.map(nama => `
                            <div class="flex items-center gap-2 bg-rose-50 dark:bg-rose-950/30 p-2 rounded-lg border border-rose-100 dark:border-rose-900/40">
                                <span class="text-[11px] font-bold text-rose-700 dark:text-rose-400">${nama}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>`;
            ...
            const sectionOff = `
                <div>
                    <h5 class="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1.5 flex items-center gap-1"><i data-lucide="calendar-off" class="w-3.5 h-3.5"></i> Off / Izin / Sakit (${tidakHadir.length})</h5>
                    <div class="space-y-1.5">
                        ${tidakHadir.length === 0 ? '<p class="text-[10px] text-slate-400 pl-1">Tidak ada.</p>' : tidakHadir.map(x => `
                            <div class="flex items-center justify-between gap-2 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-100 dark:border-slate-800">
                                <span class="text-[11px] font-bold text-slate-600 dark:text-slate-300">${x.nama}</span>
                                <span class="text-[9px] font-bold text-slate-400 uppercase shrink-0">${x.jenis}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>`;

            container.innerHTML = `<p class="text-[10px] text-slate-400 mb-1">Rekap kurir pada ${tglCek}:</p>` + sectionSudah + sectionBelum + sectionOff;
            if (window.lucide) lucide.createIcons();
        };

        // Popup sederhana: daftar transaksi mitra yang diinput seorang kurir pada tanggal tertentu.
        window.tampilkanDetailTrxKurir = function(namaKurir, tglCek) {
            const existing = document.getElementById('popup-detail-trx-kurir');
            if (existing) existing.remove();

            const rows = [];
            for (let k in cloudLogMitra) {
                const log = cloudLogMitra[k];
                if (!log || log.tglRaw !== tglCek) continue;
                if (log.kurirNama !== namaKurir) continue;
                rows.push(log);
            }
            rows.sort((a, b) => (a.waktu || '').localeCompare(b.waktu || ''));

            const totalTrx = rows.reduce((sum, r) => sum + (parseInt(r.trxInput) || 0), 0);

            const popup = document.createElement('div');
            popup.id = 'popup-detail-trx-kurir';
            popup.className = 'fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4';
            popup.innerHTML = `
                <div class="bg-white dark:bg-darkCard w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-4 space-y-3 max-h-[80vh] overflow-y-auto">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="font-bold text-sm text-slate-800 dark:text-white">${namaKurir}</h4>
                            <p class="text-[10px] text-slate-400">${tglCek} • Total ${totalTrx} Trx dari ${rows.length} mitra</p>
                        </div>
                        <button onclick="document.getElementById('popup-detail-trx-kurir').remove()" class="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500"><i data-lucide="x" class="w-3.5 h-3.5"></i></button>
                    </div>
                    <div class="space-y-1.5">
                        ${rows.length === 0 ? '<p class="text-[11px] text-slate-400 text-center py-4">Tidak ada data.</p>' : rows.map(log => `
                            <div class="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-2 rounded-lg text-[11px] border border-slate-100 dark:border-slate-700/50">
                                <span class="font-bold text-slate-700 dark:text-slate-200">${log.mitraNama || '-'}</span>
                                <span class="text-teal-600 dark:text-teal-400 font-bold shrink-0">${log.trxInput || 0} Trx • ${log.waktu || ''}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        window.renderAdminDaftarMitra = function() {
            ...
            try {
                ...
                for (let key in cloudMitraList) {
                    ...
                    const kategoriLoginAdmin = ((userSession && userSession.kategori) || '').trim();
                    const bolehLihatTglBergabung = isOwner || kategoriLoginAdmin === 'Petugas Kemitraan';
                    const tglBergabungHtml = (bolehLihatTglBergabung && m.tglBergabung)
                        ? `<div class="text-[10px] text-slate-400 mt-0.5">Bergabung: <span class="font-bold text-slate-600 dark:text-slate-300">${m.tglBergabung}</span></div>`
                        : '';
      ...
      window.hitungOngkirAdminNormal = function() {
        ...
        window.applyManajemenAccess = function(kategori) {
            ...
            if (kategoriFixed === 'Petugas Kemitraan') {
                // Petugas Kemitraan memakai screen & navigasi terpisah (screen-kemitraan-*),
                // jadi tidak perlu menyentuh menu-menu admin di sini.
                return;
            }
